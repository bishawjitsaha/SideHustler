...
const PostPage = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        setLoading(true);
        const response = await axios.get(`https://localhost:3000/posts/${id}`, {
          headers: {
  ...
  const handleApply = async () => {
    try {
      const response = await axios.get(
        `https://localhost:3000/posts/applicant-add/${id}`,
        {
          headers: {
            Authorization: `Bearer ${currentUser.accessToken}`,
          },
  ...
  const handleRemoveApplicant = async () => {
    try {
      const response = await axios.get(
        `https://localhost:3000/posts/applicant-remove/${id}`,
        {
          headers: {
            Authorization: `Bearer ${currentUser.accessToken}`,
          },
  ...
  const handleChooseApplicant = async (applicantId) => {
    try {
      const res = await axios.put(`https://localhost:3000/posts/${post._id}`, { selectedApplicant: applicantId }, {
        headers: {
          Authorization: `Bearer ${currentUser.accessToken}`
        }
      });
  ...
  const handleUnchooseApplicant = async () => {
    try {
      const res = await axios.put(`https://localhost:3000/posts/${post._id}`, { selectedApplicant: null }, {
        headers: {
          Authorization: `Bearer ${currentUser.accessToken}`
        }
      });
  ...
  const handleCompleteTask = async () => {
    try{
      const res = await axios.put(`https://localhost:3000/posts/update-status/${post._id}`, { status: "completed" }, {
            headers: {
                Authorization: `Bearer ${currentUser.accessToken}`
            }
        });
  ...
  const handleRateApplicant = async (rating) => {
    try {
        const res = await axios.post(
            `https://localhost:3000/user/update-rating/${chosenApplicant}`,
            { rating }
        );
