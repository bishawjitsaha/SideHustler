...
function ProfilePage() {
    ...
    const fetchData = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.get(`https://localhost:3000/user/${username}`, {
                headers: {
    ...
    return (
        <div className='flex justify-center'>
            {user && (
                <div>
                    <h1 className='font-bold'>{user.firstName.charAt(0).toUpperCase() + user.firstName.slice(1)} {user.lastName.charAt(0).toUpperCase() + user.lastName.slice(1)}</h1>
                    <p>Contact: {user.email} </p>
                    <img src={user.pfp !== "" ? user.pfp : "https://braverplayers.org/wp-content/uploads/2022/09/blank-pfp.png"} alt='Profile' className='w-32 h-32 rounded-full' />
